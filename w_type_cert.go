...
type X509 struct {
	// Использования ключа. Может быть установлен в следующие значения::
	...
	// Пользователь ключа (По этому параметру можно определить, держатель сертификата (ключа) является юр. лицом или нет). Следует отметить, что это поле является массивом и может содержать сразу несколько значений.
	//  Возможные значения:
